...
uint32_t appTxDutyCycle = 60000; /* 60 second */
...
static void prepareDataFrame(void)
{
  uint8_t i;
  uint8_t resultMain;
  uint16_t temperature = 0, huminity = 0;
  uint16_t co2 = 0xFFFF;
  uint32_t lux = 0;
  ...
  if (resultMain == node.ku8MBSuccess) {
    ...
    if (resultMain == node.ku8MBSuccess) {
      ...
      lux <<= 16;
      lux += node.getResponseBuffer(0x01);
  ...
  appData[i++] = (uint8_t)(huminity >> 8);
  appData[i++] = (uint8_t)(huminity & 0xFF);
  appData[i++] = (uint8_t)(lux >> 24);
  appData[i++] = (uint8_t)(lux >> 16);
  appData[i++] = (uint8_t)(lux >>  8);
  appData[i++] = (uint8_t)(lux >>  0);
  if (0xFFFF != co2) {
    appData[i++] = (uint8_t)(co2 >> 8);
    appData[i++] = (uint8_t)(co2 >> 0);
  }
  appDataSize = i;
}
...
void setup() {
  ...
  swser.begin(9600);
  node.begin(16, swser); /* modbus slave id 16 */
  nodeCO2.begin(1, swser); /* modbus slave id 1 */

  //Serial1.begin(9600);
  //node.begin(16, Serial1); /* modbus slave id 16 */
